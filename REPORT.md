...
- **Redaction (one-way, for outputs)** — one function, `Guardrail.redact`, applied everywhere real data reaches an output: JSONL persistence *and* console `print()` (found live that only the log was covered, not the terminal). Patterns cover the mock bank's structured IDs (`ACC-`, `SUB-`, `TXN-`, `CONF-`) and dollar amounts. **Deliberate scope decision:** the bare `member_id` is *not* redacted (matches the assignment's own open usage of `"12345"`) — these are hand-picked patterns for this app's ID shapes, not a general PII detector.
- **Tokenization (two-way, for the LLM call)** — `SensitiveValueTokenizer` (`comp_use/tokenizer.py`), reusing the same `redaction_patterns` list `Guardrail.redact()` reads. `DiscoveryAgent` tokenizes `observed.accessibility_tree` before it reaches `decide_next_action()`, so the model only ever reasons over placeholder tokens (`[[TOK1]]`, ...) — the raw values never leave the machine on this path. The decision the model returns is detokenized back to real values at a single boundary (locator, target, text) before it drives `surface.act()`, gets risk-classified, or is persisted into the artifact — everything downstream of that boundary behaves exactly as if tokenization never happened. Same-value-same-token within a run, so the model can still recognize "I've seen this account before." **Field-agnostic by construction:** neither `SensitiveValueTokenizer` nor `Guardrail.redact()` has any per-field logic — both just iterate `redaction_patterns`. Today that list matches account/sub-account/transaction/confirmation IDs and dollar amounts because that's the only sensitive content the mock bank actually renders; covering a new field shape (SSN, email, phone, ...) in a different app is one regex added to `Settings.redaction_patterns`, picked up by both the LLM-facing tokenization *and* the output redaction from that single change — not a code change to either consumer. Verified in `tests/test_tokenizer.py` (a synthetic SSN + email pattern added at test time, no production code touched) and `tests/test_discovery_agent.py::test_sensitive_values_are_tokenized_to_the_llm_but_real_to_the_browser` (the real discovery-loop path: the LLM-facing tree never contains the raw value, but the actual browser action and the persisted `Step` do). **Known gap, not covered:** the vision fallback still sends a raw screenshot to a vision-capable model — pixels can't be tokenized by regex; that channel remains an open exposure, mitigated only by pointing `LLMClient` at a local/on-prem model.
...
Hosted OpenRouter still receives tokenized text on `discover` (not raw UI content) and a raw screenshot only on the vision-fallback path. Point `LLMClient` at a local model to close that remaining channel for production locality.
